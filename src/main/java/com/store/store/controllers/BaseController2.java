...
import com.store.store.utils.mappers.MapperBaseController2;
...
public abstract class BaseController2<E extends BaseEntity, S extends BaseService<E>, UnsavedDTO,
        SavedDTO extends BaseSavedDTO> {
    @Autowired
    private S service;
    private MapperBaseController2<E, UnsavedDTO, SavedDTO> mapper =
            new MapperBaseController2(getClass());
    ...
    @PostMapping("")
    public ResponseEntity<ResponseDTO> create(@Valid @RequestBody UnsavedDTO unsavedDTO) {
        E entity = mapper.getMapperTypes().mapToEntity(unsavedDTO);
        E savedEntity = service.create(entity);
        return ResponseEntity.status(HttpStatus.OK).body(mapper.mapToDTO(savedEntity));
    }

    @PutMapping("")
    public ResponseEntity<ResponseDTO> update(@Valid @RequestBody SavedDTO savedDTO) {
        E entity = mapper.getMapperTypes().mapToEntity(savedDTO);
